...
def fetch_data(symbol: str, timeframe: str, period: str = "5d"):
    """Obtiene datos de yfinance y calcula los indicadores requeridos."""
    try:
        ...
        # Cálculo de EMAs
        df['EMA_30'] = ta.ema(df['Close'], length=config.EMA_FAST)
        df['EMA_50'] = ta.ema(df['Close'], length=config.EMA_MID)
        df['EMA_100'] = ta.ema(df['Close'], length=config.EMA_SLOW)
...
def analyze_symbol(symbol: str):
    """
    Evalúa la triple confirmación (H4, H1, M5).
    Retorna la señal ('COMPRA', 'VENTA' o None) y la información técnica asociada.
    """
    df_h4 = fetch_data(symbol, "1h", "1mo") # H4 aproximado con 1h/4h
